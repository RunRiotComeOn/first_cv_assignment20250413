...
### 5. 网格参数可视化 `visualize_weights_and_biases.ipynb`

本文件的主要功能如下：

- **数据加载**：从 CIFAR-10 数据集中加载图像和标签，归一化到 [0, 1] 范围。
- **模型加载**：从指定的 `.npz` 文件加载预训练的 ConvNet 模型权重。
- **单张图像分析**：
  - 预测图像的类别。
  - 可视化卷积层和池化层的特征图。
  - 显示所有类别的概率分布。
- **特征可视化**：可视化指定层的特征图，展示模型如何处理输入图像。
- **权重和偏置可视化**：可视化卷积层、全连接层和批归一化参数的权重和偏置。

代码结构如下介绍：
1. **`load_cifar10_batch(file)`**  
   - **功能**：从 CIFAR-10 数据文件中加载一批图像和标签。
   - **输入**：数据文件路径。
   - **输出**：归一化的图像数据（形状：`(n, 3, 32, 32)`）和对应的标签。

2. **`visualize_features(features, layer_name)`**  
   - **功能**：可视化指定层的所有特征图。
   - **输入**：
     - `features`：特征图，CuPy 数组（形状：`(n, c, h, w)`）。
     - `layer_name`：层名称（例如 `'conv1'`）。
   - **输出**：特征图的可视化图像。

3. **`analyze_single_image(model, image, true_label, model_file)`**  
   - **功能**：分析单张图像，包括预测类别、可视化特征图和显示类别概率分布。
   - **输入**：
     - `model`：预训练的 ConvNet 模型。
     - `image`：输入图像。
     - `true_label`：图像的真实标签。
     - `model_file`：模型权重文件路径。
   - **输出**：
...
4. **`visualize_weights_and_biases(model, model_file)`**  
   - **功能**：可视化 ConvNet 模型的权重和偏置，包括卷积滤波器、全连接层权重和批归一化参数。
   - **输入**：
     - `model`：ConvNet 模型。
     - `model_file`：包含模型权重的 `.npz` 文件路径。
   - **输出**：
     - 卷积权重（`w1`、`w2`）以滤波器图像形式展示。
     - 偏置（`b1`、`b2`、`b3`）的条形图。
     - 全连接层权重（`w3`）的热力图。
     - 批归一化参数（`gamma1`、`beta1`、`gamma2`、`beta2`）的条形图。

5. **`main()`**  
   - **功能**：协调工作流程，加载模型和数据，调用可视化和分析函数。
   - **执行**：加载预训练模型并可视化其权重和偏置。
